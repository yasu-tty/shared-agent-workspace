...
`feature-planning` は、複数領域にまたがる変更、仕様変更、高リスク変更、長時間かかる変更について、`plans/*.md` の実装計画を作成または更新する skill です。このディレクトリは Claude Code から使うための adapter で、共有 plan 仕様と coordination rules に従います。
