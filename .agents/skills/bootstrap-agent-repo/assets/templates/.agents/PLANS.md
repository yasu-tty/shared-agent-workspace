...
## Review findings rules

- Review feedback / review result を受け取る、または対象 plan のレビュー結果を作成する場合は、口頭応答だけで完了せず、active plan または checkpoint に記録する。
- ユーザーが read-only review only を明示した場合は、PLAN 更新を行わず、その制約を応答に明記する。
- 各 finding について、採用 / 見送り / 保留、対応要否、対応内容または見送り理由、次担当、target files、validation results、handoff checkpoint を追跡する。
- `Decision / status` は `accepted`, `rejected`, `deferred`, `pending`, `done` など、現在状態が分かる短い値にする。
- 既存 plan に `Review findings` section がない場合は、少なくとも `Work file activity` と `Progress log` に同等の内容を残す。
- Review 対応で scope expansion が必要な場合は、non-plan file を編集する前に `Scope`, `Ownership and coordination`, `Routing and execution`, `Agreement matrix`, `Work file activity` を更新する。
- `State` を変更するときは、未完了の review finding が `Status`, `Remaining work`, `Work file activity`, `Validation`, `Handoff checkpoint` と矛盾しないか確認する。
