...
古い skill 更新を既存 repo に再適用する場合は、まず `README.md` の再適用フローを使い、`--check-root-sync` と再適用後のバックアップ比較まで実施する。
...
Bootstrap 後、multi-agent work の前に `docs/agent/OWNERSHIP.md` の placeholder を repo 用に置き換え、`docs/agent/HANDOFF_PROTOCOL.md` を確認する。
